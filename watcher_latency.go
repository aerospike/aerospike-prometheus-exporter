	...
	for ns, stats := range rawStats {
		for opType, statsDetails := range stats {
			...
			for i, label := range statsDetails.(StatsMap)["buckets"].([]string) {
				...
				ch <- prometheus.MustNewConstMetric(pm.desc, pm.valueType, math.Floor(statsDetails.(StatsMap)["valBuckets"].([]float64)[i]), rawMetrics["cluster-name"], rawMetrics["service"], ns, opType, label, fmt.Sprintf("> %4s", labelValue), *tags)
			}
			...
			ch <- prometheus.MustNewConstMetric(pm.desc, pm.valueType, val, rawMetrics["cluster-name"], rawMetrics["service"], ns, opType, metricName, fmt.Sprintf("< %4s", metricValue), *tags)
			...
			ch <- prometheus.MustNewConstMetric(pm.desc, pm.valueType, val, rawMetrics["cluster-name"], rawMetrics["service"], ns, opType, "Total", "total", *tags)
		}
	}
